...
export default function CommunityLivePage() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      ...
      router.push(`/speakspace/${data.roomName}`);
    } catch (err: any) {
      alert(err.message);
    } finally {
      setCreating(false);
  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      if (data.status === 'matched') {
        ...
        setTimeout(() => router.push(`/speakspace/${data.roomName}`), 1500);
      } else {
          ...
          try {
            ...
            if (qRes.ok) {
              ...
              if (qData.status === 'matched') {
                ...
                if (matchPollRef.current) clearInterval(matchPollRef.current);
                setTimeout(() => router.push(`/speakspace/${qData.match?.roomName || qData.roomName}`), 1500);
              }
            }
          } catch {}
        }, 3000);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1 text-sm sm:text-base ml-28">
              Practice English with real people, live
            </p>
          </div>
          {canCreateRooms ? (
            <Button
              ...
              <CardContent className="p-6 relative">
                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-white text-lg">Find a Speak Partner</h3>
                  ...
                  <div className="flex flex-wrap items-center gap-3 lg:ml-auto">
                    ...
                    {matchStatus === 'idle' ? (
                      <Button onClick={handleMatch} className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white shadow-lg shadow-green-500/20">
                        <Zap className="w-4 h-4 mr-1.5" /> Find Partner
                      </Button>
...
function RoomCard({ room, currentUserId, isAdmin }: { room: LiveRoom; currentUserId?: string; isAdmin?: boolean }) {
  ...
  return (
    <Link href={`/speakspace/${room.roomName}`} className="block">
